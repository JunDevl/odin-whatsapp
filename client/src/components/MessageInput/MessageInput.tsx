...
const MessageInput = ({ kind }: Props) => {
  ...
  return (
    <footer className="z-20 px-1 pb-1" id="input-area">
      <form id="message-form" method="POST" onSubmit={onSubmitMessage} ref={form} className="overflow-hidden bg-dark-300 rounded-4xl has-[textarea:focus]:outline-1 has-[textarea:focus]:outline-light-900">
        <label className="flex items-stretch gap-2 py-0.5" htmlFor="content">
          <button id="send" className={`flex items-center justify-center w-11 aspect-square rounded-4xl ${text !== "" ? "bg-primary-900" : ""}`}>
