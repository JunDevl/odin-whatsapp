...
const MessageInput = ({ kind }: Props) => {
  ...
  return (
    <footer className="z-20 px-1 pb-1" id="input-area">
      <form id="message-form" method="POST" onSubmit={onSubmitMessage} ref={form} className="bg-dark-300 rounded-4xl has-[textarea:focus]:outline-1 has-[textarea:focus]:outline-light-900">
        <label className="flex items-stretch p-0.5 gap-2" htmlFor="content">
          <button id="send" className={`flex items-center p-2 px-3 size-10 rounded-4xl ${text !== "" ? "bg-primary-900" : ""}`}>
          ...
          <textarea 
            className="min-w-0 flex-1 field-sizing-content focus:outline-0"
            name="content" 
            id="content" 
            ref={input}
            value={text}
